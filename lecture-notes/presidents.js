...
//create array that filters post 1800s

function main(){
   console.table (presidents.filter(pres => pres.year >= 1900)); // Variable

   let newObj = []; // conditional
   presidents.forEach(pres1 =>{
    if(pres1.year >= 1900){
        newObj.push(pres1);
    }
   });
//    console.table(newObj);
   ...
   let newObj1 = [];//for loop
   for(let i = 0; i < presidents.length;i++){
    if(presidents[i].year >= 1900){
        newObj1.push(presidents[i]);
    }
   }
//    console.table(newObj1);
   newObj1.forEach(pres1 => console.log(`${pres1.first} ${pres1.last}`));
}
